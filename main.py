...
#Some stuff is still on experimental
...
#experimental
@client.command(help = 'experimental command')
async def hello(ctx):
  greetings = ['Hello', 'hi', 'sup', 'wadup']
  await ctx.channel.send(f'{random.choice(greetings)}')

@client.command(help='experimental command')
async def hru(ctx):
  answers = ['I\'m fine', 'I am fine as hell', 'I am very much fine']
  await ctx.channel.send(f'{random.choice(answers)}')

@client.command(name='you-ok', help='experimental command')
async def u_ok(ctx):
  words = ['Yea', 'No']
  await ctx.channel.send(f'{random.choice(words)}')

@client.command(help='experimental calculator')
